...
TConjunto interseccionDeConjuntos(TConjunto c1, TConjunto c2){
    ...
    TIterador i3 = crearIterador();
    while(estaDefinidaActual(i1) && estaDefinidaActual(i2)){
        if(actualEnIterador(i1) == actualEnIterador(i2)){
            i3 = agregarAIterador(actualEnIterador(i1), i3);
            i1 = avanzarIterador(i1);
            i2 = avanzarIterador(i2);
        }
        else if(actualEnIterador(i1) > actualEnIterador(i2))
            avanzarIterador(i2);
        else //actualEnIterador(i1) < actualEnIterador(i2)
            avanzarIterador(i1);
    }
    
    nat tamanio = cantidadEnIterador(i3); 
    i3 = reiniciarIterador(i3);
