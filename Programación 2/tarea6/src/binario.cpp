...
static double sumaUltimosPositivosAux(nat i, TBinario b, nat &cont);
static TBinario binarioBalanceadoAux(ArregloNats elems, int desde, int hasta);
...
TBinario binarioBalanceadoAux(ArregloNats elems, int desde, int hasta){
    if(hasta < desde) return NULL;
    else{
        TBinario res = new _rep_binario;
        int mitad = (desde+hasta)/2;
        res->elem = crearInfo(elems[mitad], 0);
        res->izq = binarioBalanceadoAux(elems, desde, mitad-1);
        res->der = binarioBalanceadoAux(elems, mitad+1, hasta);
        return res;
    }
}
...
TBinario avlABinario(TAvl a){
    if(a == NULL) return NULL;
        ...
        TBinario res = new _rep_binario;
        res->elem = crearInfo(raizAvl(a), 0);
